...
class AsyncPipelineEngine:
    ...
    async def run_with_healing(
        ...
        """
        Self-healing agentic loop: plan → execute → on failure, replan & retry.
        Wraps the plan/run/replan cycle so callers don't need to manage it manually.
        
        自我修復的 Agentic 迴圈：規劃 → 執行 → 失敗時自動重新規劃並重試。
        將 plan/run/replan 循環封裝，呼叫者無需手動管理。
        
        Args:
            planner: A PipelinePlanner instance (accepts duck-typed to avoid circular imports).
            query: The user's original task description.
            max_retries: Maximum number of retry attempts (including the initial run).
        
        Returns:
            EngineResult: The final execution result after all attempts.
        """
        current_plan = planner.plan(query)
        
        for attempt in range(1, max_retries + 1):
            logger.info(f"\n🔄 Agentic Loop: attempt {attempt}/{max_retries}")
            
            result = await self.run(current_plan)
            
            if result.is_success:
                logger.info(f"✅ Agentic Loop: task completed on attempt {attempt}")
                return result
            
            logger.warning(f"⚠️ Agentic Loop: attempt {attempt} failed — {len(result.failed_tasks)} node(s)")
            
            if attempt < max_retries:
                logger.info("🚨 Agentic Loop: triggering dynamic re-planning (Self-Healing)...")
        ...
        logger.error(f"❌ Agentic Loop: all {max_retries} attempts exhausted")
        return result
